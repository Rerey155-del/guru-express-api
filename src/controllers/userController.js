...
const login = async (req, res) => {
  ...
  try{
    ...
    // Support both Bcrypt hash and plain-text (for legacy users like 'admin')
    let isMatch = false;
    ...
    res.json({ token, user: { id: user.id, username: user.username, role: user.role || 'Admin', full_name: user.full_name }});
    }
    catch(error){
      res.status(500).json({ message: error.message });
    }
